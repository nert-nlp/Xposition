            ...
            for words in [sent['swes'], sent['smwes'], sent['wmwes']]:
                for i in words:
                    if words[i]['lexcat'] in ['P', 'PRON.POSS', 'POSS', 'PP', 'INF.P']:
                        ...
                        self.construal_id = query.construal(role_name, function_name, special, self.sent_id, raise_error=self.missing_con_error)
                        ...
                        if self.save_ptok and int(self.construal_id) > 0 and int(self.usage_id) > 0 and int(self.adposition_id) > 0:
                            ...
                            self.main_subtoken_indices = self.main_indices(token_indices)
                            self.main_subtoken_string = self.main_string(self.mwe_subtokens, token_indices)
                            self.add_ptoken()
                        # else:
                        ...
                        role_id = query.supersense(role_name, self.sent_id, raise_error=self.missing_ss_error)
                        function_id = query.supersense(function_name, self.sent_id, raise_error=self.missing_ss_error)
                        if self.save_con and not (role_name, function_name, special) in self.construal_set and (
                            (int(role_id) > 0 and int(function_id) > 0) or special):
                            ...
                            self.construal_set.add((role_name, function_name, special))
                        if self.save_us and not (self.adposition_name, role_name, function_name, special) in self.usage_set and \
                            ...
                            self.usage_set.add((self.adposition_name, role_name, function_name, special))
                        if self.save_ss and role_name.strip() and not role_name in self.supersense_set:
                            self.supersense_json.append({
                                'supersense_name': role_name
                            })
                            self.supersense_set.add(role_name)
                        if self.save_ss and function_name.strip() and not function_name in self.supersense_set:
                            self.supersense_json.append({
                                'supersense_name': function_name
                            })
                            self.supersense_set.add(function_name)

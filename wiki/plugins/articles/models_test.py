...
class ArticlePlugin(models.Model):
    ...
    objects = managers.ArticlePluginManager()
    ...
    def get_children(self, max_num=None, user_can_read=None, **kwargs):
        ...
        for obj in self.articlepluginforobject_set.filter(is_mptt=True):
            ...
            for child in objects.order_by(
                    'articles__articleplugin__current_revision__title'):
                cnt += 1
...
class ArticlePluginForObject(models.Model):
    ...
    objects = managers.ArticlePluginFkManager()
...
@python_2_unicode_compatible
class ArticlePluginRevision(BaseRevisionMixin, models.Model):
...
pre_save.connect(on_article_revision_pre_save, ArticlePluginRevision)
post_save.connect(on_article_revision_post_save, ArticlePluginRevision)
post_save.connect(on_article_save_clear_cache, ArticlePlugin)
pre_delete.connect(on_article_delete_clear_cache, ArticlePlugin)

...
class ArticlePlugin(models.Model):
    ...
    current_revision = models.OneToOneField(
        'ArticlePluginRevision', verbose_name=_('current revision'),
        blank=True, null=True, related_name='current_set',
        help_text=_(
            'The revision being displayed for this articleplugin. If you need to do a roll-back, simply change the value of this field.'),)
    ...
    modified = models.DateTimeField(
        ...
        help_text=_('ArticlePlugin properties last modified'))

    owner = models.ForeignKey(
        compat.USER_MODEL, verbose_name=_('owner'),
        blank=True, null=True, related_name='owned_articleplugins',
        help_text=_(
            'The owner of the articleplugin, usually the creator. The owner always has both read and write access.'),
    ...
    def ancestor_objects(self):
        ...
        for obj in self.articlepluginforobject_set.filter(is_mptt=True):
    ...
    def get_children(self, max_num=None, user_can_read=None, **kwargs):
        ...
        for obj in self.articlepluginforobject_set.filter(is_mptt=True):
            ...
            for child in objects.order_by(
                    'articleplugins__articleplugin__current_revision__title'):
                cnt += 1
    ...
    def set_permissions_recursive(self):
        for descendant in self.descendant_objects():
            if descendant.INHERIT_PERMISSIONS:
                descendant.articleplugin.group_read = self.group_read
                descendant.articleplugin.group_write = self.group_write
                descendant.articleplugin.other_read = self.other_read
                descendant.articleplugin.other_write = self.other_write
                descendant.articleplugin.save()

    def set_group_recursive(self):
        for descendant in self.descendant_objects():
            if descendant.INHERIT_PERMISSIONS:
                descendant.articleplugin.group = self.group
                descendant.articleplugin.save()

    def set_owner_recursive(self):
        for descendant in self.descendant_objects():
            if descendant.INHERIT_PERMISSIONS:
                descendant.articleplugin.owner = self.owner
                descendant.articleplugin.save()
    ...
    def add_revision(self, new_revision, save=True):
        ...
        assert self.id or save, (
            'ArticlePlugin.add_revision: Sorry, you cannot add a'
            'revision to an articleplugin that has not been saved '
            'without using save=True')
        if not self.id:
            self.save()
        revisions = self.articlepluginrevision_set.all()
        try:
            new_revision.revision_number = revisions.latest(
            ).revision_number + 1
        except ArticlePluginRevision.DoesNotExist:
            new_revision.revision_number = 0
        new_revision.articleplugin = self
    ...
    def add_object_relation(self, obj):
        ...
        rel = ArticlePluginForObject.objects.get_or_create(articleplugin=self,
    ...
    def get_for_object(cls, obj):
        return ArticlePluginForObject.objects.get(
            object_id=obj.id,
            content_type=ContentType.objects.get_for_model(obj)).articleplugin

    def __str__(self):
        if self.current_revision:
            return self.current_revision.title
        obj_name = _('ArticlePlugin without content (%(id)d)') % {'id': self.id}
        return str(obj_name)

    class Meta:
        permissions = (
            ("moderate", _("Can edit all articleplugins and lock/unlock/restore")),
            ("assign", _("Can change ownership of any articleplugin")),
    ...
    def get_absolute_url(self):
        ...
        if urlpaths.exists():
            ...
            return reverse('wiki:get', kwargs={'articleplugin_id': self.id})


@python_2_unicode_compatible
class ArticlePluginForObject(models.Model):

    objects = managers.ArticleFkManager()

    articleplugin = models.ForeignKey('ArticlePlugin', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return "{}".format(self.articleplugin)

    class Meta:
        verbose_name = _('ArticlePlugin for object')
        verbose_name_plural = _('ArticlePlugins for object')
        # Do not allow several objects
        unique_together = ('content_type', 'object_id')


class BaseRevisionMixinPlugin(models.Model):
    ...
    def inherit_predecessor(self, predecessor):
        ...
        object.current_revision implements BaseRevisionMixinPlugin.
        """
...
@python_2_unicode_compatible
class ArticlePluginRevision(BaseRevisionMixinPlugin, models.Model):

    """This is where main revision data is stored. To make it easier to
    copy, do NEVER create m2m relationships."""

    articleplugin = models.ForeignKey('ArticlePlugin', on_delete=models.CASCADE,
                                verbose_name=_('articleplugin'))

    # This is where the content goes, with whatever markup language is used
    content = models.TextField(blank=True, verbose_name=_('articleplugin contents'))

    # This title is automatically set from either the articleplugin's title or
    # the last used revision...
    title = models.CharField(
        max_length=512, verbose_name=_('articleplugin title'),
        null=False, blank=False,
        help_text=_(
            'Each revision contains a title field that must be filled out, even if the title has not changed'))

    # TODO:
    # Allow a revision to redirect to another *articleplugin*. This
    # way, we can redirects and still maintain old content.
    # redirect = models.ForeignKey('ArticlePlugin', null=True, blank=True,
    #                             verbose_name=_('redirect'),
    #                             help_text=_('If set, the articleplugin will redirect to the contents of another articleplugin.'),
    #                             related_name='redirect_set')

    def __str__(self):
        return "%s (%d)" % (self.title, self.revision_number)

    def inherit_predecessor(self, articleplugin):
        """
        Inherit certain properties from predecessor because it's very
        convenient. Remember to always call this method before
        setting properties :)"""
        predecessor = articleplugin.current_revision
        self.articleplugin = predecessor.articleplugin
    ...
    class Meta:
        ...
        ordering = ('created',)
        unique_together = ('articleplugin', 'revision_number')
...
# clear the ancestor cache when saving or deleting articleplugins so things like
# articleplugin_lists will be refreshed
def _clear_ancestor_cache(articleplugin):
    for ancestor in articleplugin.ancestor_objects():
        ancestor.articleplugin.clear_cache()

@disable_signal_for_loaddata
def on_articleplugin_save_clear_cache(instance, **kwargs):
    on_articleplugin_delete_clear_cache(instance, **kwargs)


@disable_signal_for_loaddata
def on_articleplugin_delete_clear_cache(instance, **kwargs):
    _clear_ancestor_cache(instance)
    instance.clear_cache()


@disable_signal_for_loaddata
def on_articleplugin_revision_pre_save(**kwargs):
    instance = kwargs['instance']
    if kwargs.get('created', False):
        revision_changed = (
            not instance.previous_revision and
            instance.articleplugin and
            instance.articleplugin.current_revision and
            instance.articleplugin.current_revision != instance
        )
        if revision_changed:
            instance.previous_revision = instance.articleplugin.current_revision

    if not instance.revision_number:
        try:
            previous_revision = instance.articleplugin.articlepluginrevision_set.latest()
...
@disable_signal_for_loaddata
def on_articleplugin_revision_post_save(**kwargs):

    instance = kwargs['instance']
    if not instance.articleplugin.current_revision:
        # If I'm saved from Django admin, then articleplugin.current_revision is
        # me!
        instance.articleplugin.current_revision = instance
        instance.articleplugin.save()


pre_save.connect(on_articleplugin_revision_pre_save, ArticlePluginRevision)
post_save.connect(on_articleplugin_revision_post_save, ArticlePluginRevision)
post_save.connect(on_articleplugin_save_clear_cache, ArticlePlugin)
pre_delete.connect(on_articleplugin_delete_clear_cache, ArticlePlugin)
